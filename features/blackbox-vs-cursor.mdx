...
On a given task, while BlackBox makes a clear plan of action for implementation and asks for user feedback.
...
Cursor's context window may fill up after reading a few big files leading to summarization and loss of context, especially when the changes are big.
...
Cursor tries to use the terminal and CURL commands to get the code or rendered page which will not help in robust testing or in debugging a runtime error.
...
**Cursor Issues:**
- Took long to make initial changes
