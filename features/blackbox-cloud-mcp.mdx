...
description: "Integrate with remote agents via Model Context Protocol (MCP) to Blackbox CLI for seamless terminal-based task management and execution."
---
...
Once connected, you can use commands or use natural language commands in the Blackbox CLI to control remote agents. The MCP integration supports querying tools, managing tasks, executing operations and do much more seamlessly.

- **Explore Available Tools**  
  Use the Keyboard short-cut "Ctrl + t" or just simply ask the CLI: `"print all the remote-code tools"` to list all available tools and their capabilities.  
...
- **Start a New Task**  
  ...
  ![Start Task](/images/blackbox-cloud/cloud_mcp_start_task.png)
  You can verify the same via UI
...
- **Task Orchestration**: Enable the CLI agent to start, create, and manage one-to-many sub-tasks or parallel tasks, orchestrating them to achieve final goals through task creation, stopping, and merging changes.
...
- **Automated Task Management**: Let the CLI handle task orchestration and management instead of manual oversight.
