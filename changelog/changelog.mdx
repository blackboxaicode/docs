...
<Update label="Oct 30" description="Full Self Coding">
  **New Full Self Coding Feature**

  What's New

  - Connect BLACKBOX AI agents directly to your production servers for real-time monitoring and autonomous error resolution.
  - Agents continuously monitor server logs, detect errors as they occur, and automatically implement fixes with comprehensive testing.
  - Select from multiple specialized agents (Blackbox CLI, Claude Code, Codex) to handle different types of tasks and issues.
  - Each implementation goes through rigorous testing workflows including unit tests, integration tests, regression tests, and security scans.
  - Automated pull request generation with detailed change logs and test evidence for seamless code review and deployment.
  - Real-time dashboard provides live visibility into error detection, agent activity, task queue, and test results.
  - Checkout the detailed documentation for [Full Self Coding](/features/full-self-coding)
</Update>
