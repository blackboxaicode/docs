...
<Update label="Oct 30" description="Full Self Coding">
  **New Full Self Coding Feature**
  ...
  - Connect AI agents directly to your production servers for real-time error monitoring and autonomous debugging.
  - Agents continuously fetch server logs in the background, detecting and surfacing errors as they occur.
  - Select from multiple specialized agents (Debugging, Performance, Security, General Purpose) optimized for different error scenarios.
  - Comprehensive testing pipeline ensures every implementation is stable before deployment, including unit tests, integration tests, and build verification.
  - Real-time visibility into agent decision-making, execution logs, and test results for complete transparency.
  - Enables self-healing production environments with minimal human intervention while maintaining oversight and control.
  - Checkout the detailed documentation for the [Full Self Coding](/features/full-self-coding)
</Update>
