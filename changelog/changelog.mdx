...
<Update label="Oct 24" description="Model Context Protocol (MCP) Support in BLACKBOX Cloud">
  **New Model Context Protocol (MCP) Integration in BLACKBOX Cloud**

  What's New

  - Seamless integration with Model Context Protocol (MCP) for enhanced AI agent capabilities.
  - Enables direct connection to external data sources and tools through standardized MCP interfaces.
  - Supports dynamic context sharing between AI models and external applications.
  - Allows agents to access real-time data from databases, APIs, and file systems via MCP servers.
  - Improves agent performance with contextual awareness from integrated development environments.
  - Facilitates extensible workflow automation through MCP-compatible tools and services.
  - Checkout the detailed documentation for the [MCP Integration](/features/blackbox-cloud-mcp)
</Update>
...
<Update label="Oct 7" description="Launch of BLACKBOX AI Remote">
  **Launch of BLACKBOX AI Remote**
  ...
  - Supports running various agent types including Blackbox CLI, Cloud Code, and Codeex agents.
