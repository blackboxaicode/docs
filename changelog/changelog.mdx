...
<Update label="Oct 24" description="Model Context Protocol (MCP) Integration">
  **New MCP Integration for BLACKBOX AI Remote Agents**

  What's New

  - Terminal-based interaction with remote agents via Model Context Protocol (MCP) using Blackbox CLI.
  - Assign tasks, monitor progress, and control remote agents directly from the command line.
  - Natural language commands for seamless task management without leaving your development environment.
  - Token-based authentication for secure remote access to Blackbox Cloud MCP server.
  - CLI agent can orchestrate multiple sub-tasks and parallel tasks using remote agents.
  - Tool discovery feature to dynamically list and utilize available remote capabilities.
